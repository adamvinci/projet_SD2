...
public class Graph {
    ...
    void afficherChemin(Map<String, Troncon> tronconMap, String sommetCourant, String stationArrivee,
        ...
        List<Troncon> troncons1 = new ArrayList<>();
        while (iterator.hasPrevious()) {
            Troncon troncon = iterator.previous();
            System.out.println(troncon);
            troncons1.add(troncon);
        }
        ...
        List<String> strings = troncons1.stream().map(troncon -> troncon.getLigne().getNom()).distinct().toList();

        HashMap<String, Long> nbTroncons = new HashMap<>();

        for (String s : strings) {
            Long nb = cheminTroncon.stream().filter(troncon -> troncon.getLigne().getNom().equals(s)).count();
            nbTroncons.put(s, nb);
        }

        for (String s : strings) {
            troncons1.stream().filter(troncon -> troncon.getLigne().getNom().equals(s)).findFirst().ifPresent(
                    troncon -> {
                        Deplacement deplacement = new Deplacement(s, troncon.getDepart(), troncon.getArrivee(),
                                troncon.getLigne().getAttenteMoyenne(), troncon.getDuree(), Math.toIntExact(nbTroncons.get(s)),
                                troncon.getLigne().getType(), troncon.getLigne().getDestination());
